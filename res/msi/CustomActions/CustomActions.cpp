...
void RecursiveDelete(LPCWSTR path, MSIHANDLE hInstall)
{
    ...
    do
    {
        ...
        if (findData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
        {
            ...
            {
                WcaLog(LOGMSG_STANDARD, "RecursiveDelete: Skipping reparse point (symlink/junction): %ls", fullPath);
                if (FALSE == RemoveDirectoryW(fullPath))
                {
                    WcaLog(LOGMSG_STANDARD, "RecursiveDelete: Failed to remove reparse point. Error: %lu", GetLastError());
                }
            }
            else
            {
